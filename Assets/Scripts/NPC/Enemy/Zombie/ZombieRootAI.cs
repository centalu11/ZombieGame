...
{
    /// <summary>
    /// Root motion AI controller for zombies with parameter-based animation control
    /// </summary>
    public class ZombieRootAI : MonoBehaviour
    {
        [Header("AI Settings")]
        ...
        [Tooltip("Speed multiplier for root motion (1.0 = normal speed)")]
        [Range(0.1f, 3f)]
        public float rootMotionSpeedMultiplier = 1f;
        
        [Tooltip("Apply root motion to Y axis (vertical movement)")]
        public bool applyRootMotionY = false;
        ...
        [Tooltip("Disable manual movement for root motion (movement comes from animation)")]
        public bool disableManualMovement = true;
        ...
        private void InitializeComponents()
        {
            ...
            if (_animator == null)
            {
                Debug.LogError("[ZombieRootAI] No Animator component found!");
                return;
            }
        ...
        {
            // For root motion, we only use static wandering for timing, not movement
            bool isMoving = staticWandering.UpdateWandering(0f, disableManualMovement);
            ...
            if (_animator != null)
            {
                if (isMoving)
                {
                    ...
                    // Set animation speed with root motion multiplier
                    _animator.speed = wanderWalkAnimation.GetAnimationSpeed() * rootMotionSpeedMultiplier;
                }
                else if (isWaitingBetweenSteps)
                {
        ...
        private void HandleChasingState(float distanceToPlayer)
        {
            ...
            // Calculate direction to player for animation decision making
            Vector3 direction = (_player.position - transform.position).normalized;
            direction.y = 0; // Keep direction horizontal
            
            // Handle animation - root motion will handle movement and rotation
            HandleRootMotionChasing();
        }
        
        /// <summary>
        /// Handle chasing with root motion
        /// </summary>
        private void HandleRootMotionChasing()
        {
            if (_animator == null) return;
            
            // Just play the animation - root motion will handle movement
            ...
            // Set animation speed multiplier for root motion
            float speedMultiplier = useWalkForChasing ? chaseWalkAnimation.GetAnimationSpeed() : chaseRunAnimation.GetAnimationSpeed();
            _animator.speed = speedMultiplier * rootMotionSpeedMultiplier;
        }
        ...
        public void ValidateAnimations()
        {
            ...
            {
                Debug.LogWarning($"[ZombieRootAI] {gameObject.name}: {missingCount} animation(s) are missing assignments!");
            }
        }
        ...
        public void TestHeadAndArmsUpward()
        {
            // Get animator if not already set
            if (_animator == null)
            {
                _animator = GetComponent<Animator>();
            }
            ...
            // Auto-detect head bone
            Transform testHeadBone = _animator?.GetBoneTransform(HumanBodyBones.Head);
            if (testHeadBone == null)
            {
                testHeadBone = FindChildByName(transform, "Head");
            }
            if (testHeadBone == null)
            {
                testHeadBone = FindChildByName(transform, "head");
            ...
            if (testHeadBone == null)
            {
                testHeadBone = FindChildByName(transform, "Neck");
            }
            if (testHeadBone == null)
            {
                testHeadBone = FindChildByName(transform, "neck");
            ...
            // Auto-detect arm bones
            Transform testLeftArmBone = _animator?.GetBoneTransform(HumanBodyBones.LeftUpperArm);
            if (testLeftArmBone == null)
            {
                testLeftArmBone = FindChildByName(transform, "LeftArm");
            }
            if (testLeftArmBone == null)
            {
                testLeftArmBone = FindChildByName(transform, "leftArm");
            ...
            if (testLeftArmBone == null)
            {
                testLeftArmBone = FindChildByName(transform, "L_Arm");
            }
            
            Transform testRightArmBone = _animator?.GetBoneTransform(HumanBodyBones.RightUpperArm);
            if (testRightArmBone == null)
            {
                testRightArmBone = FindChildByName(transform, "RightArm");
            }
            if (testRightArmBone == null)
            {
                testRightArmBone = FindChildByName(transform, "rightArm");
            ...
            if (testRightArmBone == null)
            {
                testRightArmBone = FindChildByName(transform, "R_Arm");
            }
            
            // Test head rotation
            if (testHeadBone != null)
            {
                Vector3 testHeadRotation = testHeadBone.localEulerAngles;
                testHeadRotation.x = -45f; // Look up 45 degrees
                testHeadBone.localRotation = Quaternion.Euler(testHeadRotation);
            }
            else
            {
                Debug.LogWarning($"[ZombieAI] {gameObject.name}: No head bone found for testing!");
            }
            
            // Test left arm rotation
            if (testLeftArmBone != null)
            {
                Vector3 testLeftArmRotation = testLeftArmBone.localEulerAngles;
                testLeftArmRotation.x = -45f; // Point up 45 degrees
                testLeftArmBone.localRotation = Quaternion.Euler(testLeftArmRotation);
            }
            else
            {
                Debug.LogWarning($"[ZombieAI] {gameObject.name}: No left arm bone found for testing!");
            }
            
            // Test right arm rotation
            if (testRightArmBone != null)
            {
                Vector3 testRightArmRotation = testRightArmBone.localEulerAngles;
                testRightArmRotation.x = -45f; // Point up 45 degrees
                testRightArmBone.localRotation = Quaternion.Euler(testRightArmRotation);
            }
            else
            {
                Debug.LogWarning($"[ZombieAI] {gameObject.name}: No right arm bone found for testing!");
            }
        }
