...
namespace ZombieGame.Player.Animations
{
    /// <summary>
    ...
    public class EyeAnimationHandler : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            // Start the first blink with random interval
            float firstInterval = CalculateRandomBlinkInterval();
            Invoke(nameof(AnimateEyes), firstInterval);
        }

        /// <summary>
        /// Calculates the next random blink interval using the complex 3-level randomization system
        /// </summary>
        /// <returns>The next blink interval in seconds</returns>
        private float CalculateRandomBlinkInterval()
        {
            // Level 1: 50% chance for fixed interval, 50% chance for different
            bool useFixed = Random.Range(0f, 1f) < 0.5f;
            
            if (useFixed)
            {
                // Debug.Log($"EyeAnimationHandler: Using fixed interval: {blinkInterval}s");
                return blinkInterval;
            }
            
            // Level 2: 50% chance for shorter, 50% chance for longer
            bool useShorter = Random.Range(0f, 1f) < 0.5f;
            
            float finalInterval;
            if (useShorter)
            {
                // Level 3 (Shorter): Subtract 1-3 seconds from base interval
                float subtractAmount = Random.Range(1f, 3f);
                finalInterval = Mathf.Max(0.5f, blinkInterval - subtractAmount); // Minimum 0.5s
                // Debug.Log($"EyeAnimationHandler: Using shorter interval: {finalInterval}s (base: {blinkInterval}s - {subtractAmount}s)");
            }
            else
            {
                // Level 3 (Longer): Add 2-5 seconds to base interval
                float addAmount = Random.Range(3f, 5f);
                finalInterval = blinkInterval + addAmount;
                // Debug.Log($"EyeAnimationHandler: Using longer interval: {finalInterval}s (base: {blinkInterval}s + {addAmount}s)");
            }
        ...
        /// <summary>
        /// Manual blink function that can be called from external scripts
        /// </summary>
        public void Blink()
        {
            if (CanAnimate)
            {
                // Debug.Log("EyeAnimationHandler: Manual blink triggered");
                AnimateEyes();
            }
        }
        ...
        private async void AnimateEyes()
        {
            ...
            // Schedule the next blink with a new random interval
            float nextInterval = CalculateRandomBlinkInterval();
            Invoke(nameof(AnimateEyes), nextInterval);
        }
